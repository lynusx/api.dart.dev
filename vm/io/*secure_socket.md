...
更多信息请参见 [Socket](https://www.yuque.com/thyname/dart.io/socket)。
...
返回的 Future 将以已连接且可供订阅的 [SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 完成。

服务器提供的证书将使用 SecurityContext 对象中设置的受信任证书进行检查。默认的 SecurityContext 对象包含一组内置的、针对知名证书颁发机构的受信任根证书。

[onBadCertificate] 是用于处理无法验证的证书的可选处理程序。该处理程序接收 [X509Certificate](https://www.yuque.com/thyname/dart.io/x509certificate)，可以检查它并决定（或让用户决定）是否接受该连接。该处理程序应返回 true 以继续 [SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 连接。
...
参数 [timeout] 用于指定等待建立连接所允许的最长时间。如果 [timeout] 长于系统级别的超时时长，超时可能会比 [timeout] 指定的时间更早发生。超时时，将抛出 [SocketException](https://www.yuque.com/thyname/dart.io/socketexception)，并取消所有正在进行的、连接到 [host] 的连接尝试。
...
类似于 [connect]，但返回一个 [Future](https://www.yuque.com/thyname/dart.async/future)，该 Future 以一个 [ConnectionTask](https://www.yuque.com/thyname/dart.io/connectiontask) 完成，如果不再需要 [SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket)，可以取消该任务。
...
接受一个已连接的 [socket]，并启动客户端侧的 TLS 握手，以使通信安全。当返回的 Future 完成时，[SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 已完成 TLS 握手。使用此函数要求连接的另一端已准备好进行 TLS 握手。

如果 [socket] 已经有订阅，此订阅将不再接收任何事件。在大多数情况下，在启动 TLS 握手之前，对该订阅调用 [StreamSubscription.pause] 是正确的做法。

如果传入了 [host] 参数，它将用作 TLS 握手的主机名。如果未传入 [host]，将使用 [socket] 的主机名。[host] 可以是 [String](https://www.yuque.com/thyname/dart.core/string) 或 [InternetAddress](https://www.yuque.com/thyname/dart.io/internetaddress)。
...
调用此函数**不会**触发 DNS 主机名查找。如果传入的 [host] 是 [String](https://www.yuque.com/thyname/dart.core/string)，生成的 [SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 的 [InternetAddress](https://www.yuque.com/thyname/dart.io/internetaddress) 将以传入的 [host] 作为其主机值，并以已连接套接字的网际地址作为其地址值。
...
接受一个已连接的 [socket]，并启动服务器侧的 TLS 握手，以使通信安全。当返回的 Future 完成时，[SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 已完成 TLS 握手。使用此函数要求连接的另一端即将启动 TLS 握手。
...
如果此 [SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 是安全套接字连接的服务器端，[peerCertificate] 将返回客户端证书；如果未收到客户端证书，则返回 `null`。如果此套接字是客户端，[peerCertificate] 将返回服务器的证书。
...
通过调用 RawSecureSocket.connect 提供到服务器的客户端连接。使用 [RawSecureServerSocket](https://www.yuque.com/thyname/dart.io/rawsecureserversocket) 创建的安全服务器也会返回代表安全连接服务器端的 `RawSecureSocket` 对象。服务器提供的证书将使用 [SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext) 对象中设置的受信任证书进行检查。默认的 [SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext) 对象包含一组内置的、针对知名证书颁发机构的受信任根证书。

更多信息请参见 [RawSocket](https://www.yuque.com/thyname/dart.io/rawsocket)。
...
返回的 [Future](https://www.yuque.com/thyname/dart.async/future) 在连接完成且可供订阅时以 [RawSecureSocket](https://www.yuque.com/thyname/dart.io/rawsecuresocket) 完成。
...
[onBadCertificate] 是用于处理无法验证的证书的可选处理程序。该处理程序接收 [X509Certificate](https://www.yuque.com/thyname/dart.io/x509certificate)，可以检查它并决定（或让用户决定）是否接受该连接。该处理程序应返回 true 以继续 [RawSecureSocket](https://www.yuque.com/thyname/dart.io/rawsecuresocket) 连接。
...
类似于 [connect]，但返回一个 [Future](https://www.yuque.com/thyname/dart.async/future)，该 Future 以一个 [ConnectionTask](https://www.yuque.com/thyname/dart.io/connectiontask) 完成，如果不再需要 [RawSecureSocket](https://www.yuque.com/thyname/dart.io/rawsecuresocket)，可以取消该任务。
...
接受一个已连接的 [socket]，并启动客户端侧的 TLS 握手，以使通信安全。当返回的 Future 完成时，[RawSecureSocket](https://www.yuque.com/thyname/dart.io/rawsecuresocket) 已完成 TLS 握手。使用此函数要求连接的另一端已准备好进行 TLS 握手。
...
接受一个已连接的 [socket]，并启动服务器侧的 TLS 握手，以使通信安全。当返回的 Future 完成时，[RawSecureSocket](https://www.yuque.com/thyname/dart.io/rawsecuresocket) 已完成 TLS 握手。使用此函数要求连接的另一端即将启动 TLS 握手。
