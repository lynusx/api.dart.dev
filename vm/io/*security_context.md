...
[SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 和 [SecureServerSocket](https://www.yuque.com/thyname/dart.io/secureserversocket) 类将 SecurityContext 作为其 connect 和 bind 方法的参数。
...
创建一个新的 [SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext)。

默认情况下，创建的 [SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext) 不包含任何密钥或证书。可以通过调用此类的方法来添加它们。

如果 [withTrustedRoots] 传入 `true`，[SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext) 将使用下文所述提供的受信任根证书进行初始化。要获取包含受信任根证书的 [SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext)，通常使用 [SecurityContext.defaultContext] 就已足够，应优先使用它。但是，如果必须针对每个连接修改包含受信任根证书的 [SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext)，则应使用 [withTrustedRoots]。
...
向 [SecureSocket](https://www.yuque.com/thyname/dart.io/securesocket) 客户端连接使用的受信任 X509 证书集合中添加证书。
...
设置 [SecureServerSocket](https://www.yuque.com/thyname/dart.io/secureserversocket) 在建立安全连接时提供的 X509 证书链，包括服务器证书。
...
设置 [SecureServerSocket](https://www.yuque.com/thyname/dart.io/secureserversocket) 在向连接的客户端请求客户端证书时，将通告为可接受的颁发机构名称列表。
...
如果为 `true`，[SecurityContext](https://www.yuque.com/thyname/dart.io/securitycontext) 将允许 TLS 重新协商。重新协商仅在作为客户端时受支持，并且 HelloRequest 必须在应用协议的空闲点收到。这足以支持在（非流水线的）HTTP/1.1 中，在一个 HTTP 请求和响应之间请求新客户端证书的旧用例。注意：重新协商是一个存在严重问题的协议特性，应仅在已知安全的环境中用于与旧版服务器通信。
...
如果对端不支持 `minimumTlsProtocolVersion` 或更高版本，[SecureSocket.connect] 将抛出 [TlsException](https://www.yuque.com/thyname/dart.io/tlsexception)。
