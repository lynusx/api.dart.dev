...
更底层的 [listen] 方法是其他所有方法的基础。你在流上调用 `listen`，告诉它你想要接收事件，并注册用于接收这些事件的回调函数。调用 `listen` 时，你会得到一个 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription) 对象，它是提供事件的活动对象，可用于停止监听，或临时暂停来自该订阅的事件。
...
[isBroadcast] 的默认实现返回 `false`。继承自 [Stream](https://www.yuque.com/thyname/dart.async/stream) 的广播流必须重写 [isBroadcast]，使其返回 `true`，以表明自己表现得像一个广播流。
...
每次监听所创建的流时，都会以一个新的 [MultiStreamController](https://www.yuque.com/thyname/dart.async/multistreamcontroller) 调用 [onListen] 回调，该控制器将事件转发给该次 [listen] 调用所返回的 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription)。

这使得每个监听者都可以被当作一个独立的流来处理。

[MultiStreamController](https://www.yuque.com/thyname/dart.async/multistreamcontroller) 不支持读取其 [StreamController.stream]。设置其 [StreamController.onListen] 没有任何效果，因为会改为调用 [onListen] 回调，且之后不会再调用 [StreamController.onListen]。该控制器的行为类似于异步控制器，但提供了额外的方法用于同步地传递事件。
...
[period] 必须是一个非负的 [Duration](https://www.yuque.com/thyname/dart.core/duration)。
...
当返回的流被监听时，会调用给定的 [mapSink] 闭包。来自 [source] 的所有事件都被添加到该次调用返回的事件 sink 中。转换过程会将所有转换后的事件放入 [mapSink] 闭包在调用时收到的 sink 中。从概念上讲，[mapSink] 创建了一个转换管道，输入 sink 是返回的 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink)，输出 sink 是它接收到的那个 sink。
...
如果该 [Stream](https://www.yuque.com/thyname/dart.async/stream) 为空或包含多个元素，返回的 future 将以错误完成。
...
返回一个 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription)，它使用提供的 [onData]、[onError] 和 [onDone] 处理程序处理来自该流的事件。这些处理程序可以在订阅上更改，但它们最初就是提供的这些函数。
...
否则，它会仅以错误对象作为参数被调用。如果省略了 [onError]，该流上的任何错误都会被视为未处理，并传递给当前 [Zone](https://www.yuque.com/thyname/dart.async/zone) 的错误处理程序。默认情况下，未处理的异步错误会被当作未捕获的顶级错误处理。
...
其行为类似于 [map]，[convert] 函数对每个数据事件调用一次，但此处 [convert] 可以是异步的，并返回一个 [Future](https://www.yuque.com/thyname/dart.async/future)。如果发生这种情况，该流会在继续处理后续事件之前等待该 future 完成。
...
返回一个新流，该流中该流的每个元素都被替换为零个或多个数据事件。事件值通过以该元素为参数调用 [convert] 得到，返回的是一个 [Iterable](https://www.yuque.com/thyname/dart.core/iterable)，其元素按迭代顺序发出。如果调用 [convert] 抛出异常，或者对返回值的迭代抛出异常，该错误会在返回的流上发出，并且该流中该元素的迭代随之结束。
...
当该流的所有元素都已处理完毕时，返回的 [Future](https://www.yuque.com/thyname/dart.async/future) 完成。
...
将该流的所有元素收集到一个 [List](https://www.yuque.com/thyname/dart.core/list) 中。
...
将该流的数据收集到一个 [Set](https://www.yuque.com/thyname/dart.core/set) 中。
...
如果在找到该值之前发生 done 事件，future 将以一个 [RangeError](https://www.yuque.com/thyname/dart.core/rangeerror) 完成。
...
调用 [onTimeout] 函数时会带有一个参数：一个允许向返回的流中放入事件的 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink)。这个 `EventSink` 仅在 [onTimeout] 调用期间有效。在传递给 [onTimeout] 的 sink 上调用 [EventSink.close] 会关闭返回的流，并且不再处理进一步的事件。

如果省略了 [onTimeout]，超时会在返回的流的错误通道中发出一个 [TimeoutException](https://www.yuque.com/thyname/dart.async/timeoutexception)。如果对 [onTimeout] 的调用抛出异常，该错误会作为返回流上的一个错误发出。
...
对来自 [Stream](https://www.yuque.com/thyname/dart.async/stream) 的事件的订阅。

当你使用 [Stream.listen] 监听一个 [Stream](https://www.yuque.com/thyname/dart.async/stream) 时，会返回一个 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription) 对象。
...
该 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription) 当前是否处于暂停状态。
...
[handleError] 函数必须能够以一个位置参数调用，或者以两个位置参数调用（其中第二个参数始终是 [StackTrace](https://www.yuque.com/thyname/dart.core/stacktrace)）。
...
支持添加错误的 [Sink](https://www.yuque.com/thyname/dart.core/sink)。

这使得它适合用于捕获异步计算的结果，这些结果可以以一个值或一个错误完成。

[EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 的设计目的是处理来自 [Stream](https://www.yuque.com/thyname/dart.async/stream) 的异步事件。例如参见 [Stream.eventTransformed]，它使用 `EventSink` 来转换事件。
...
只暴露 [Stream](https://www.yuque.com/thyname/dart.async/stream) 接口的 [Stream](https://www.yuque.com/thyname/dart.async/stream) 包装器。
...
[StreamSink](https://www.yuque.com/thyname/dart.async/streamsink) 结合了来自 [StreamConsumer](https://www.yuque.com/thyname/dart.async/streamconsumer) 和 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 的方法。

在调用 [addStream] 期间，不能使用 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 的方法。一旦 [addStream] 返回的 [Future](https://www.yuque.com/thyname/dart.async/future) 以某个值完成，就可以再次使用 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 的方法。

如果在任何 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 方法之后调用 [addStream]，它将被延迟，直到底层系统消费完通过 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 方法添加的数据。

当使用 [EventSink](https://www.yuque.com/thyname/dart.async/eventsink) 的方法时，可以使用 [done] [Future](https://www.yuque.com/thyname/dart.async/future) 来捕获任何错误。

当调用 [close] 时，它将返回 [done] [Future](https://www.yuque.com/thyname/dart.async/future)。
...
返回一个 future，当该 [StreamSink](https://www.yuque.com/thyname/dart.async/streamsink) 完成时完成。
...
当在流上调用 [Stream.transform] 方法并传入一个 [StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer) 时，该流会在提供的转换器上调用 [bind] 方法。所产生的流随后由 [Stream.transform] 方法返回。

从概念上讲，转换器只是一个从 [Stream](https://www.yuque.com/thyname/dart.async/stream) 到 [Stream](https://www.yuque.com/thyname/dart.async/stream) 的函数，被封装成一个类。

编写可以多次使用的转换器是一种良好的实践。

`Stream` 上所有其他的转换方法，例如 [Stream.map]、[Stream.where] 或 [Stream.expand]，都可以使用 [Stream.transform] 来实现。因此，[StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer) 非常强大，但通常使用起来也稍微更复杂一些。
...
基于给定的 [onListen] 回调创建一个 [StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer)。

返回的流转换器在转换后的流被监听时使用提供的 [onListen] 回调。此时，该回调会收到输入流（传递给 [bind] 的那个流）以及一个布尔标志 `cancelOnError`，用于创建一个 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription)。
...
最常见的情况是，[onListen] 回调会先在提供的流上调用 [Stream.listen]（带有相应的 `cancelOnError` 标志），然后返回一个新的 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription)。

创建 StreamSubscription 有两种常见方式：

1.  分配一个 [StreamController](https://www.yuque.com/thyname/dart.async/streamcontroller)，并返回监听其流的结果。重要的是要转发暂停、恢复和取消事件（除非该转换器有意想改变此行为）。
2.  创建一个实现 [StreamSubscription](https://www.yuque.com/thyname/dart.async/streamsubscription) 的新类。请注意，该订阅应在流被监听时所在的 [Zone](https://www.yuque.com/thyname/dart.async/zone) 中运行回调（参见 [Zone](https://www.yuque.com/thyname/dart.async/zone) 和 [Zone.bindCallback]）。
...
创建一个将事件委托给给定函数的 [StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer)。
...
基于一个 [bind] 回调创建一个 [StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer)。
...
[StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer) 接口是完全通用的，因此它无法说明子类具体做了什么。每个 [StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer) 都应（在访问该转换器的类或变量上）清楚地说明它是如何转换流的，以及与以下典型行为的任何差异：
...
用于实现 [StreamTransformer](https://www.yuque.com/thyname/dart.async/streamtransformer) 的基类。
...
一个类似于 [Iterator](https://www.yuque.com/thyname/dart.core/iterator) 的接口，用于 [Stream](https://www.yuque.com/thyname/dart.async/stream) 的值。

它包装了一个 [Stream](https://www.yuque.com/thyname/dart.async/stream) 及对该流的一个订阅。它监听该流，并在下一个值可用时完成 [moveNext] 返回的 future。
...
在 [stream] 上创建一个 [StreamIterator](https://www.yuque.com/thyname/dart.async/streamiterator)。
...
如果 StreamIterator 尚未移动到第一个元素（[moveNext] 尚未被调用并完成），或者 StreamIterator 已经移动到超过最后一个元素之后（[moveNext] 返回了 `false`），则 [current] 的值未指定。此时 [StreamIterator](https://www.yuque.com/thyname/dart.async/streamiterator) 可以选择抛出异常，或返回一个特定于该迭代器实现的默认值。
