...
An _asynchronous computation_ cannot provide a result immediately when it is started, unlike a synchronous computation which does compute a result immediately by either returning a value or by throwing. An asynchronous computation may need to wait for something external to the program (reading a file, querying a database, fetching a web page) which takes time. Instead of blocking all computation until the result is available, the asynchronous computation immediately returns a `Future` which will _eventually_ "complete" with the result.
...
If a future completes with an _error_, awaiting that future will (re-)throw that error. In the example here, we can add error checking:
...
In general, when writing asynchronous code, you should always await a future when it is produced, and not wait until after another asynchronous delay. That ensures that you are ready to receive any error that the future might produce, which is important because an asynchronous error that no-one is awaiting is an _uncaught_ error and may terminate the running program.

### Programming with the `Future` API.

The `Future` class also provides a more direct, low-level functionality for accessing the result that it completes with. The `async` and `await` language features are built on top of this functionality, and it sometimes makes sense to use it directly. There are things that you cannot do by just `await`ing one future at a time.
...
Like [Future.delayed], but does not perform any action, and cannot complete with an error.
...
Non-[Future] return values, and completion-values of returned [Future]s, are discarded.
...
Creates a new _timeout future_ that completes with the same result as this future, the _source future_, _if_ the source future completes in time.
...
If [handleError] throws, the returned future completes with the thrown error and stack trace, except that if it throws the _same_ error object again, then it is considered a "rethrow" and the original stack trace is retained. This can be used as an alternative to skipping the error in [test]. Example:
