...
[Future]s and [Stream]s are the fundamental building blocks of asynchronous programming in Dart. They are supported directly in the language through `async` and `async*` functions, and are available to all libraries through the `dart:core` library.

This library provides further tools for creating, consuming and transforming futures and streams, as well as direct access to other asynchronous primitives like [timers][Timer], [microtasks][scheduleMicrotask] and [zones][Zone].
...
Many methods in the Dart libraries return `Future`s when performing tasks. For example, when binding an `HttpServer` to a host and port, the `bind()` method returns a Future.
