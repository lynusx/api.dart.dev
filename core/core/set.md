...
```dart
Set<T> castFrom<S, T>(Set<S> source, {Set<R> Function<R>()? newSet})
```
...
If the equality relation used by the set is not identity, then the returned object may not be _identical_ to [object]. Some set implementations may not be able to implement this method. If the [contains] method is computed, rather than being based on an actual object instance, then there may not be a specific object instance representing the set element.
