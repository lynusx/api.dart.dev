...
The `Function` class is a supertype of all _function types_, and contains no values itself. All objects that implement `Function` have a function type as their runtime type.
...
However, a value with the static type `Function` _can_ still be called like a function.
...
```dart
apply(Function function, List<dynamic>? positionalArguments, [Map<Symbol, dynamic>? namedArguments])
```
...
- Instantiations of equal generic functions with the _same_ types yields equal results.
...
A compiler can convert the closure `(x, y) => x - y` into a top-level function.
