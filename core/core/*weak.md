...
[Expando](https://www.yuque.com/thyname/dart.core/expando) 允许为对象添加新的属性。
...
创建一个新的 [Expando](https://www.yuque.com/thyname/dart.core/expando)。可选的 name 参数仅用于调试目的，使用相同名称创建两个不同的 [Expando](https://www.yuque.com/thyname/dart.core/expando) 会得到两个作用于不同属性的 [Expando](https://www.yuque.com/thyname/dart.core/expando)（即它们所作用的对象属性彼此独立）。
...
传递给构造函数的该 [Expando](https://www.yuque.com/thyname/dart.core/expando) 的名称。
...
获取该 [Expando](https://www.yuque.com/thyname/dart.core/expando) 在给定对象上的属性值。
...
将该 [Expando](https://www.yuque.com/thyname/dart.core/expando) 在给定对象上的属性值设置为 [value]。
...
并非所有对象都支持作为弱引用的目标。[WeakReference](https://www.yuque.com/thyname/dart.core/weakreference) 构造函数会拒绝任何不支持作为 [Expando](https://www.yuque.com/thyname/dart.core/expando) 键的对象。
...
创建一个指向给定 [target] 的 [WeakReference](https://www.yuque.com/thyname/dart.core/weakreference)。
...
该 [WeakReference](https://www.yuque.com/thyname/dart.core/weakreference) 当前弱引用的对象（如果存在）。
...
在 Dart 原生运行时上运行时，如果回调是原生函数而非 Dart 函数，请改用 `dart:ffi` 的 [NativeFinalizer](https://www.yuque.com/thyname/dart.ffi/nativefinalizer)。
...
[callback] 会在 [Finalizer](https://www.yuque.com/thyname/dart.core/finalizer) 创建时绑定到当前 zone，并在被调用时在该 zone 中运行。
...
[value] 和 [detach] 参数不会使这些对象被视为可被程序访问。两者都必须是支持作为 [Expando](https://www.yuque.com/thyname/dart.core/expando) 键的对象，且可以是**同一个**对象。
