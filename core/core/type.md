...
- By a _type literal_, a type name occurring as an expression, like `Type type = int;`, or a type variable occurring as an expression, like `Type type = T;`.
...
The string should be consistent, so a `Type` object for the _same_ type returns the same string throughout a program execution. The string may or may not contain parts corresponding to the source name of declaration of the type, if the type has a source name at all (some types reachable through `dart:mirrors` may not).
