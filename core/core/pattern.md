...
The matches are found by repeatedly finding the first match of the pattern in the string, initially starting from [start], and then from the end of the previous match (but always at least one position later than the _start_ of the previous match, in case the pattern matches an empty substring).
...
Some patterns, regular expressions in particular, may record substrings that were part of the matching. These are called _groups_ in the `Match` object. Some patterns may never have any groups, and their matches always have zero [groupCount].
