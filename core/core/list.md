...
The created list is fixed-length if [growable] is false (the default) and growable if [growable] is true. If the list is growable, increasing its [length] will _not_ initialize new entries with [fill]. After being created and filled, the list is no different from any other growable or fixed-length list created using `[]` or other [List] constructors.
...
```dart
List.from(Iterable<Object?> elements, {bool growable = true})
```
...
```dart
List.unmodifiableOf(Iterable<E> elements)
```
...
Methods which accept `Object?` as argument, like [contains] and [remove], will pass the argument directly to the this list's method without any checks.
...
If `iterable` is based on this list, its values may change _during_ the `setAll` operation.
...
The returned [Iterable] behaves like `skip(start).take(end - start)`. That is, it does _not_ break if this list changes size, it just ends early if it reaches the end of the list early (if `end`, or even `start`, becomes greater than [length]).
