...
[Set](https://www.yuque.com/thyname/dart.core/set) 的默认实现 [LinkedHashSet](https://www.yuque.com/thyname/dart.collection/linkedhashset) 会将根据 [Object.==] 和 [Object.hashCode] 相等的对象视为不可区分。

遍历集合元素的顺序可能是无序的，也可能以某种方式排序。示例：

- [HashSet](https://www.yuque.com/thyname/dart.collection/hashset) 是无序的，这意味着其迭代顺序是未指定的，
- [LinkedHashSet](https://www.yuque.com/thyname/dart.collection/linkedhashset) 按元素的插入顺序迭代，
- 而像 [SplayTreeSet](https://www.yuque.com/thyname/dart.collection/splaytreeset) 这样的有序集合按元素的排序顺序迭代。

通常不允许在对集合执行操作期间修改集合（添加或删除元素），例如在调用 [forEach] 或 [containsAll] 期间。也不允许在迭代集合本身或由该集合支持的任何 [Iterable](https://www.yuque.com/thyname/dart.core/iterable)（如 [where] 和 [map] 等方法返回的对象）时修改集合。
...
创建一个空的 [Set](https://www.yuque.com/thyname/dart.core/set)。

创建的 [Set](https://www.yuque.com/thyname/dart.core/set) 是一个普通的 [LinkedHashSet](https://www.yuque.com/thyname/dart.collection/linkedhashset)。因此，它会将使用 [operator ==] 判断相等的元素视为不可区分，并要求它们具有兼容的 [Object.hashCode] 实现。
...
创建一个空的恒等 [Set](https://www.yuque.com/thyname/dart.core/set)。

创建的 [Set](https://www.yuque.com/thyname/dart.core/set) 是一个使用恒等作为相等关系的 [LinkedHashSet](https://www.yuque.com/thyname/dart.collection/linkedhashset)。
...
创建一个包含所有 [elements] 的 [Set](https://www.yuque.com/thyname/dart.core/set)。
...
创建的 [Set](https://www.yuque.com/thyname/dart.core/set) 是一个 [LinkedHashSet](https://www.yuque.com/thyname/dart.collection/linkedhashset)。因此，它会将使用 [operator ==] 判断相等的元素视为不可区分，并要求它们具有兼容的 [Object.hashCode] 实现。
...
根据 [elements] 创建一个 [Set](https://www.yuque.com/thyname/dart.core/set)。
...
根据 [elements] 创建一个不可修改的 [Set](https://www.yuque.com/thyname/dart.core/set)。
...
如果提供了 [newSet]，它将用于创建 [toSet]、[union] 返回的新集合，也用于 [intersection] 和 [difference]。如果省略 [newSet]，则默认使用 [Set](https://www.yuque.com/thyname/dart.core/set) 的默认构造函数创建新集合，此时 [intersection] 和 [difference] 会返回对源集合调用相同方法所得结果的适配版本。
...
也就是说，返回的集合包含此 [Set](https://www.yuque.com/thyname/dart.core/set) 中根据 `other.contains` 判断也属于 [other] 的所有元素。
...
也就是说，返回的集合包含此 [Set](https://www.yuque.com/thyname/dart.core/set) 的所有元素以及 [other] 的所有元素。
...
也就是说，返回的集合包含此 [Set](https://www.yuque.com/thyname/dart.core/set) 中根据 `other.contains` 判断不属于 [other] 的所有元素。
...
创建一个与此 `Set` 具有相同元素和行为的 [Set](https://www.yuque.com/thyname/dart.core/set)。
