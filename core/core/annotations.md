...
If a feature is deprecated in a superclass, it is _not_ automatically deprecated in a subclass as well. It is reasonable to remove a member from a superclass and retain it in a subclass, so it needs to be possible to deprecate the member only in the superclass.
...
Further [lints](https://dart.dev/lints) can be used to enable more warnings based on `@override` annotations.
